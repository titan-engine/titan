...
const { BrowserRuntime, BrowserLiveRuntime, verify_recording_json_with_player_png } = require(resolve(metadata.target_directory, 'titan/browser-node/titan_browser.js'));
...
for (const factory of [bytes => BrowserRuntime.with_player_png(true, bytes), bytes => BrowserLiveRuntime.with_player_png(bytes)]) {
  for (const bytes of [new Uint8Array(), original.subarray(0, 20), Buffer.from('not a PNG'), new Uint8Array(256 * 1024 + 1), png(65, 1)]) {
    assert.throws(() => factory(bytes), /player.png/);
  }
}
const procedural = new BrowserRuntime(true);
const loaded = BrowserRuntime.with_player_png(true, original);
...
const changed = BrowserRuntime.with_player_png(true, replacement);
reference(changed);
assert.deepEqual(query(changed, 'save'), query(procedural, 'save'));
assert.notEqual(capture(changed), capture(procedural));
const changedReference = capture(changed);
loaded.free(); procedural.free(); changed.free();

const live = BrowserLiveRuntime.with_player_png(replacement);
...
assert.equal(JSON.parse(verify_recording_json_with_player_png(recording, replacement)).verified, true);
assert.throws(() => verify_recording_json_with_player_png(recording, original), /mismatch|asset/i);
// Every reconstruction retains the initially decoded asset.
...
live.free();
const canonicalLive = BrowserLiveRuntime.with_player_png(original);
canonicalLive.set_control_enabled(true); reference(canonicalLive);
assert.equal(capture(canonicalLive), 'f7a298f62ad75c1c'); canonicalLive.free();

execFileSync('cargo', ['build', '--example', 'replay_rpg'], { cwd: repo, stdio: 'inherit' });
const temporary = mkdtempSync(join(tmpdir(), 'titan-rpg-png-'));
try {
  writeFileSync(join(temporary, 'player.png'), replacement);
  ...
  rmSync(join(temporary, 'player.png'));
  const missing = spawnSync(executable, [path, '--assets-dir', temporary], { cwd: temporary, encoding: 'utf8' });
  assert.notEqual(missing.status, 0); assert.match(missing.stderr, /player.png/);
} finally { rmSync(temporary, { recursive: true, force: true }); }
console.log('PNG asset WASM/native acceptance passed: identical references, changed pixels, bounded failure, retained assets and replay verification.');
