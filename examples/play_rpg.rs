    ...
    pub fn run() -> Result<(), Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--help" | "-h" => {
                    println!(
                        "play_rpg [--frames N] [--replay]\nMove with arrow keys or WASD; Escape exits.\n--frames exits after N presented GPU frames.\n--replay renders the exact completed reference walk without further simulation."
                    );
                    return Ok(());
                }
                    ...
                    event_loop
                        .create_window(
                            Window::default_attributes()
                                .with_title("Titan — procedural RPG")
