...
async function initialize() {
  ...
  function status() {
    ...
    $("reference-route").disabled = currentFrame !== 0;
  ...
  $("reference-route").addEventListener("click", action(() => {
    status(); if (currentFrame !== 0) throw new Error("Reset the controlled scene before running the reference route.");
    let frame = 0;
    for (const [name, ticks] of [["right", 2], ["down", 3], ["right", 6]]) {
      for (let tick = 0; tick < ticks; tick++) request({ type: "inject_input", frame: ++frame, actions: { [name]: { kind: "button", value: true } } });
    }
