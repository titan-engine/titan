...
def main(failures, log):
    ...
    try:
        ...
        verification=json.loads(output([str(BINARY.with_name('replay')),str(snapshot_path)],text=True))
        ...
        legacy=json.loads(output([str(BINARY.with_name('replay')),str(GAME/'tests/fixtures/recording-v1.json')],text=True))
...
if __name__ == "__main__":
    with FailureEvidence("arena-control", repo=REPO) as failures:
