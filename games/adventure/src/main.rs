...
#[cfg(not(target_arch = "wasm32"))]
use titan_adventure::game::{self, build_game, configured_inspector};
#[cfg(not(target_arch = "wasm32"))]
...
fn run_native_mode() -> Result<bool, Box<dyn std::error::Error>> {
    ...
    let mut instance = format!("adventure-{}", std::process::id());
