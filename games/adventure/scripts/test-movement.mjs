...
if (process.argv.includes('--wasm-worker')) {
  const { movement_acceptance } = createRequire(import.meta.url)(process.argv[3]);
  process.stdout.write(movement_acceptance());
} else {
  ...
  await build('cargo', ['build', '--locked', '--bin', 'acceptance', '--features', 'movement-acceptance']);
  const native = JSON.parse(await execFile(join(target, 'debug/acceptance'), [], { cwd: root, encoding: 'utf8' }));
  await build('cargo', ['build', '--locked', '--lib', '--target', 'wasm32-unknown-unknown', '--release', '--features', 'movement-acceptance']);
  const directory = await mkdtemp(resolve(tmpdir(), 'adventure-movement-'));
  try {
    ...
    assert.deepEqual(wasm, native, 'every movement fixture tick agrees between native and actual WASM');
    const ticks = Object.values(native.scenarios).reduce((sum, trace) => sum + trace.length, 0);
    console.log(`Adventure movement: ${Object.keys(native.scenarios).length} scenarios, ${ticks} states; all assertions and exact native/actual-WASM agreement passed.`);
  } finally {
