...
if (process.argv.includes('--wasm-worker')) {
  const { sequence_acceptance } = createRequire(import.meta.url)(process.argv[3]);
  ...
  try {
    ...
    assert.deepEqual(wasm, native, 'every sequence fixture tick agrees between native and actual WASM');
    const ticks = Object.values(native.scenarios).reduce((sum, trace) => sum + trace.length, 0);
    console.log(`Adventure sequence: ${Object.keys(native.scenarios).length} scenarios, ${ticks} states; all assertions and exact native/actual-WASM agreement passed.`);
  } finally {
