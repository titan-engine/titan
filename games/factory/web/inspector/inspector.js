import { bridgeResponse } from "./bridge.mjs";
...
async function initialize() {
  ...
  function commands() {
    ...
    text("commands-note", result.commands.length ? "Edit JSON arguments; the runtime validates each operation before it executes." : "No commands advertised. Enable controls to see the game's commands.");
    for (const command of result.commands) {
      ...
      caption.textContent = "JSON arguments (omit fields not used by this operation)";
      input.rows = 5;
      input.value = JSON.stringify(exampleArguments(command.name), null, 2);
      ...
      form.addEventListener("submit", action(() => {
        const args = parseArguments(input.value);
