...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if arguments.first().is_some_and(|arg| arg == "--sequence") {
        ...
        if bytes.len() > 1_048_576 {
            return Err("sequence file exceeds 1 MiB".into());
        }
        let operations: Vec<serde_json::Value> = serde_json::from_slice(&bytes)?;
        ...
        println!(
            "{}",
            serde_json::json!({"outcomes": outcomes, "state": serde_json::from_str::<serde_json::Value>(&game::status(&app))?})
        );
        return Ok(());
    }
    if run_native_mode()? {
    ...
    let mut inspector = configured_inspector(
        ...
        InspectionConfig::controlled("factory", "."),
    );
...
fn run_native_mode() -> Result<bool, Box<dyn std::error::Error>> {
    ...
    let mut instance = format!("factory-{}", std::process::id());
        ...
        && duration.is_none_or(|duration| started.elapsed() < duration)
    {
        ...
        queue.drain_with_reply(|request, reply| {
            ...
            let result = diagnostics.record_response(
                ...
                elapsed_us,
                |app, bundle| {
                    bundle.world_state["factory"] = game::diagnostic_positions(app.world());
