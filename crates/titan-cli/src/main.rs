...
fn main() -> ExitCode {
    ...
    if matches!(
        ...
        let result = execute(cli.command, &cli.project);
    ...
    let (result, success) = match execute_remote(&cli) {
        Ok(result) => result,
...
fn execute(command: CliCommand, project: &Path) -> CommandResult {
    match command {
        ...
        CliCommand::Test => run_cargo("test", project, &["test", "--workspace", "--all-targets"]),
        CliCommand::RunExample { name } => {
            run_cargo("run_example", project, &["run", "--example", name.as_str()])
        }
        _ => unreachable!("remote commands handled separately"),
    }
}

fn run_cargo(command_name: &'static str, project: &Path, arguments: &[&str]) -> CommandResult {
...
mod tests {
    ...
    fn info_is_a_structured_result() {
        let result = execute(CliCommand::Info, std::path::Path::new("."));
