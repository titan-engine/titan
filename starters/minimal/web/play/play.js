...
function updateStatus() { status.textContent = player.status(); result.textContent = ''; }
...
window.addEventListener('keydown', event => { const action = keys.get(event.key); if (!player || !running || !action || event.target.closest('input, textarea, select, button, [contenteditable]')) return; event.preventDefault(); heldKeys.set(event.code, action); syncInputs(); });
